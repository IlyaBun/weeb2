...
// Список должников (студенты, у которых средний по дисциплине ниже 4)
$debtorsSql = "SELECT s.id,
                      s.full_name,
                      ...
                      GROUP_CONCAT(
                          d.name || ' (' || ROUND(risk_subjects.subject_avg_grade, 2) || ')',
                          ', '
                      ) as risk_disciplines
               FROM students s
               ...
               JOIN (
                   ...
                   HAVING AVG(gr.grade) < 4
               ) as risk_subjects ON risk_subjects.student_id = s.id
               JOIN disciplines d ON d.id = risk_subjects.discipline_id
               WHERE s.status = 'active'";

$debtorsParams = [];

if ($filterGroup > 0) {
    $debtorsSql .= " AND s.group_id = ?";
    $debtorsParams[] = $filterGroup;
}
if ($filterDiscipline > 0) {
    $debtorsSql .= " AND risk_subjects.discipline_id = ?";
    $debtorsParams[] = $filterDiscipline;
}

$debtorsSql .= " GROUP BY s.id, s.full_name, g.name
                 ORDER BY risk_discipline_count DESC, min_subject_avg_grade ASC
                 LIMIT 20";

$debtorsStmt = $pdo->prepare($debtorsSql);
$debtorsStmt->execute($debtorsParams);
            ...
            <table class="data-table">
                <thead>
                    <tr>
                        ...
                        <th>Кол-во дисциплин &lt; 4</th>
                    </tr>
                </thead>
                <tbody>
                    <?php foreach ($debtors as $index => $debtor): ?>
                    <?php $riskTooltip = 'Дисциплины: ' . $debtor['risk_disciplines']; ?>
                    <tr>
                        <td><?= $index + 1 ?></td>
                        ...
                        <td>
                            <span
                                ...
                                data-tooltip="<?= escape($riskTooltip) ?>"
                                title="<?= escape($riskTooltip) ?>"
                            >
                                <?= number_format($debtor['min_subject_avg_grade'], 2) ?>
                            </span>
                        </td>
                        <td><?= $debtor['risk_discipline_count'] ?></td>
                    </tr>
                    <?php endforeach; ?>
                </tbody>
            </table>
