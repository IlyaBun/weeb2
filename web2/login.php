...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($login) || empty($password)) {
        ...
        if ($user && $user['role'] === 'student' && empty($user['student_id'])) {
            $error = 'Учетная запись студента не привязана к карточке';
        } elseif ($user) {
            ...
            if ($user['role'] === 'student') {
                header('Location: /index.php?module=students&student_id=' . (int)$user['student_id']);
            } else {
                header('Location: /index.php');
            }
            exit;
...
<head>
    ...
    <style>
        .login-page {
            ...
            background: linear-gradient(135deg, #004a09 0%, #0b7a18 100%);
            padding: 20px;
        }
        ...
        .form-group input:focus {
            outline: none;
            border-color: #004a09;
            box-shadow: 0 0 0 3px rgba(0, 74, 9, 0.1);
        }
        ...
        .login-btn:hover {
            ...
            box-shadow: 0 10px 20px rgba(0, 74, 9, 0.3);
        }
    ...
    <div class="login-page">
        <div class="login-card">
            ...
            <div class="demo-credentials">
                ...
                <p>Студент: <code>student</code> / <code>student123</code></p>
            </div>
        </div>
    </div>
