...
function getDB() {
    ...
    if ($pdo === null) {
        try {
            ...
            ensureUserStudentColumn($pdo);
            normalizeGradeTypes($pdo);
            ensureStudentDemoUser($pdo);
...
function initializeDatabase($pdo) {
    ...
    CREATE TABLE IF NOT EXISTS users (
        ...
        role TEXT NOT NULL DEFAULT 'teacher', -- 'admin', 'teacher' или 'student'
    ...
    CREATE TABLE IF NOT EXISTS grades (
        ...
        date TEXT NOT NULL,
        type TEXT DEFAULT 'exam', -- 'exam', 'practice' или 'lab'
...
function generateTestData($pdo) {
    ...
    $demoStudent = $pdo->query("SELECT id, full_name FROM students ORDER BY id LIMIT 1")->fetch();
    if ($demoStudent) {
        $stmt = $pdo->prepare("INSERT INTO users (login, password, role, full_name, student_id) VALUES (?, ?, ?, ?, ?)");
        $stmt->execute(['student', 'student123', 'student', $demoStudent['full_name'], $demoStudent['id']]);
    }
...
/**
 * Приведение устаревших типов контроля к актуальному набору.
 */
function normalizeGradeTypes($pdo) {
    $pdo->exec("UPDATE grades
                SET type = CASE
                    WHEN id % 2 = 0 THEN 'practice'
                    ELSE 'lab'
                END
                WHERE type = 'credit'");
}
...
/**
 * Добавление колонки связи пользователя со студентом в существующую БД.
 */
function ensureUserStudentColumn($pdo) {
    $columns = $pdo->query("PRAGMA table_info(users)")->fetchAll();
    $columnNames = array_column($columns, 'name');

    if (!in_array('student_id', $columnNames, true)) {
        $pdo->exec("ALTER TABLE users ADD COLUMN student_id INTEGER DEFAULT NULL");
    }
}

/**
 * Создание демонстрационного аккаунта студента при отсутствии.
 */
function ensureStudentDemoUser($pdo) {
    $studentUserExists = (int)$pdo->query("SELECT COUNT(*) FROM users WHERE role = 'student'")->fetchColumn();

    if ($studentUserExists > 0) {
        return;
    }

    $student = $pdo->query("SELECT id, full_name FROM students WHERE status = 'active' ORDER BY id LIMIT 1")->fetch();
    ...
    $stmt = $pdo->prepare("INSERT INTO users (login, password, role, full_name, student_id) VALUES (?, ?, ?, ?, ?)");
    $stmt->execute(['student', 'student123', 'student', $student['full_name'], $student['id']]);
}
